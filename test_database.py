#!/usr/bin/env python3
"""
Test Script für Financial Data Warehouse und KPI Calculator.
Analysiert die Datenbank und testet die KPI-Extraktion.
"""
...
def test_kpi_calculator():
    ...
    try:
        from app.data.kpi_calculator import KPICalculator
...
def test_warehouse_integration():
    """Teste die Warehouse Integration."""
    print("\n🏗️ Testing Warehouse Integration:")
    print("=" * 50)
    ...
    try:
        ...
        # Test get_quarterly_comparison
        print("📊 Testing get_quarterly_comparison for GOOGL Revenue:")
        revenue_data = warehouse.get_quarterly_comparison("GOOGL", "Revenue", quarters=8)
        
        if not revenue_data.empty:
            print(f"   ✅ Found {len(revenue_data)} quarters of revenue data:")
            for _, row in revenue_data.head().iterrows():
                values = row['values'] if row['values'] else []
                print(f"   - {row['year']}Q{row['quarter']}: {len(values)} values")
        else:
            print("   ❌ No revenue data found!")
        
        # Test get_kpis
        print("\n📈 Testing get_kpis for GOOGL:")
        kpis_df = warehouse.get_kpis("GOOGL")
        
        if not kpis_df.empty:
            print(f"   ✅ Found {len(kpis_df)} KPI records:")
            unique_kpis = kpis_df['kpi_name'].unique()
            print(f"   - Unique KPIs: {list(unique_kpis)}")
        else:
        ...
        print(f"❌ Warehouse integration test failed: {e}")
        return False
...
def main():
    ...
    print("🚀 Financial Data Warehouse Test Suite")
    print("=" * 60)
    ...
    # Test 4: KPI Calculator
    if test_kpi_calculator():
        print("✅ KPI Calculator is working correctly!")
    else:
        print("❌ KPI Calculator has issues!")
    
    # Test 5: Warehouse Integration
    if test_warehouse_integration():
        print("✅ Warehouse integration is working correctly!")
    else:
        print("❌ Warehouse integration has issues!")
